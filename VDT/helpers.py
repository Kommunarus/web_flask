...
def draw_box_label(img, bbox_cv2, box_color=(0, 255, 255), show_label=True, id = None):
 ...
 #       text_x= 'x='+str((left+right)/2)
        text_x = str(id)
        cv2.putText(img,text_x,(left,top-17), font, font_size, font_color, 1, cv2.LINE_AA)
  #      text_y= 'y='+str((top+bottom)/2)
  #      cv2.putText(img,text_y,(left,top-5), font, font_size, font_color, 1, cv2.LINE_AA)
    
    return img    

def countGate(img, bbox_cv2, frame_count, trk, count_gate):
    left, top, right, bottom = bbox_cv2[1], bbox_cv2[0], bbox_cv2[3], bbox_cv2[2]
    x1 = 0
    x2 = 1280
    y1 = 290
    ...
    x_mid = (right + left) / 2
    y_mid = (bottom + top) / 2
    y_gate = y1 + (y2 - y1)*(x_mid - x1)/(x2 - x1)
    ...
    if y_gate < y_mid:
        ...
        if x_mid > gate1[0] and x_mid <= gate1[1]:
            gate = 1
            count_gate[0] = 1 + count_gate[0]
        if x_mid > gate2[0] and x_mid <= gate2[1]:
            gate = 2
            count_gate[1] = 1 + count_gate[1]
        if x_mid > gate3[0] and x_mid <= gate3[1]:
            gate = 3
            count_gate[2] = 1 + count_gate[2]
        if x_mid > gate4[0] and x_mid <= gate4[1]:
            gate = 4
            count_gate[3] = 1 + count_gate[3]
